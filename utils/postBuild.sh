...
IMAGE_SIZE_MB=$((`stat --printf="%s" -L $build_dir/tmp/deploy/images/edison/fabmo-image-edison.ext4` / 1048576))
...
then
        echo -e "\033[31mError: image fabmo-image-edison.ext4(${IMAGE_SIZE_MB}MB) has exceeded rootfs partition settings(${EDISON_ROOTFS_MB}MB)!\033[0m"
...
cp $build_dir/tmp/deploy/images/edison/fabmo-image-edison.hddimg $build_dir/toFlash/edison-image-edison.hddimg
...
# Copy rootfs
cp $build_dir/tmp/deploy/images/edison/fabmo-image-edison.ext4 $build_dir/toFlash/edison-image-edison.ext4
...
awk '{print $1 " " $3}' $build_dir/tmp/deploy/images/edison/fabmo-image-edison.manifest > $build_dir/toFlash/package-list.txt
